...
mut_handle = "../data/brca/BRCA_total_Events.csv"
annot_handle = "../data/brca/BRCA_total_sampleSelection.csv"
...
muts = ['P.TP53 (M)', 'M.TP53 (M)', 'P.PIK3CA (M)', 'M.PIK3CA (M)', 'P.GATA3 (M)', 'M.GATA3 (M)', 'P.CDH1 (M)', 'M.CDH1 (M)', 'P.ESR1 (M)', 'M.ESR1 (M)', 
        'P.KMT2C (M)', 'M.KMT2C (M)', 'P.MAP3K1 (M)', 'M.MAP3K1 (M)', 'P.PTEN (M)', 'M.PTEN (M)', 'P.ARID1A (M)', 'M.ARID1A (M)', 'P.AKT1 (M)', 'M.AKT1 (M)'] 
         #For a faster run, we can use only the 10 most frequent mutations. The full set of mutations would include:
         #'P.TBX3 (M)', 'M.TBX3 (M)', 'P.CBFB (M)', 'M.CBFB (M)', 'P.FOXA1 (M)', 'M.FOXA1 (M)', 'P.NF1 (M)', 'M.NF1 (M)', 'P.MAP2K4 (M)', 'M.MAP2K4 (M)', 
         #'P.ERBB2 (M)', ' M.ERBB2 (M)', 'P.RB1 (M)', ' M.RB1 (M)', 'P.SPEN (M)', ' M.SPEN (M)', 'P.NCOR1 (M)', ' M.NCOR1 (M)', 'P.RUNX1 (M)', ' M.RUNX1 (M)'
...
penal_weights = utils.cross_val(dat=dat,
                               penal_fun=reg_opt.symmetric_penal4,
                               splits=lams,
                               n_folds=5,
                               m_p_corr=w_corr,
                               key = key)
...
theta, d_p, d_m= reg_opt.learn_mhn(th_init=th_init,
                                   ...
                                   dat=dat,
                                   perc_met=0.2,
                                   penal=reg_opt.symmetric_penal4,
                                   w_penal=0.001,
                                   fixed_grad = False,
                                   opt_ftol=1e-05
                                   )
