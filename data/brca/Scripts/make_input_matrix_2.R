...
## this script is a reworked version of the original MET-MHN make_input_matrix
## and takes pre-processed MSK CHORD and MET genomic and clinical data to
...
## this script was written in R 4.5.1
...
# GENIE metadata (MSK-CHORD and MSK-MET)
sampleData_CHORD <- read.delim("../Data/msk_chord_2024/data_clinical_sample.txt", comment.char="#")
sampleData_MET <- read.delim("../Data/msk_met_2021/data_clinical_sample.txt", comment.char="#")

# join the two data set in one large data frame (sample duplicates are joined)
sampleData <- full_join(sampleData_CHORD, sampleData_MET,
                      by = "SAMPLE_ID",
                      suffix = c(".CHORD", ".MET"))

common_cols <- intersect(colnames(sampleData_CHORD), colnames(sampleData_MET))
common_cols <- setdiff(common_cols, "SAMPLE_ID")

for (col in common_cols) {
  sampleData[[col]] <- coalesce(
    sampleData[[paste0(col, ".CHORD")]],
    sampleData[[paste0(col, ".MET")]]
  )
}
...
# load mutation files
muts_CHORD <- read.delim("../Data/msk_chord_2024/data_mutations.txt")
muts_MET <- read.delim("../Data/msk_met_2021/data_mutations.txt")

# load annotated mutation file
# old: muts <- read.delim("data_preparation/data_mutations_annotated_eventFilter.txt")
